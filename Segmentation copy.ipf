...
Function segCorr()
	...
	display/W=(35,45,598,326)/K=1 corrmap_hist
...
function segment()
	...
	string input, inlist
	variable pmax=inf,minthr=0.2, thr=0.4,minsize=3
	inlist=wavelist("*",";","DIMS:3")
...
function roiCor(w)
	...
	for(i=0;i<l;i+=1)
		...
		for(j=0;j<(i+1);j+=1)
			...
			if (ROIcorr[i][j]>0.8 && ROIcorr[i][j]!=1)
				concerns[k][0]=i
				concerns[k][1]=j
